"""
GeoNames US — place name resolution layer.
Source: https://download.geonames.org/export/dump/US.zip
License: CC BY 4.0
"""
import io
import zipfile
import requests
from tqdm import tqdm
from pipeline.base import BaseSourceAdapter, H3_R7, H3_R9
from pipeline.db import upsert


SOURCE_URL = "https://download.geonames.org/export/dump/US.zip"

COLUMNS = [
    "geoname_id", "name", "ascii_name", "alternate_names",
    "latitude", "longitude", "feature_class", "feature_code",
    "country_code", "cc2", "admin1_code", "admin2_code",
    "admin3_code", "admin4_code", "population", "elevation_m",
    "dem", "timezone", "modification_date",
]

# Only load place-relevant feature classes
WANTED_CLASSES = {"A", "P", "S", "H", "L", "T"}


class GeoNamesUSAdapter(BaseSourceAdapter):
    source_id = "geonames_us"
    source_name = "GeoNames United States"
    license = "CC BY 4.0"

    def discover(self) -> list[str]:
        return [SOURCE_URL]

    def fetch(self, url: str) -> bytes:
        if self.is_cached(url):
            print(f"  [cache] {url}")
            return self.read_cache(url)
        print(f"  Downloading {url} ...")
        resp = requests.get(url, stream=True, timeout=120)
        resp.raise_for_status()
        data = resp.content
        self.write_cache(url, data)
        return data

    def parse(self, raw: bytes, url: str) -> list[dict]:
        records = []
        with zipfile.ZipFile(io.BytesIO(raw)) as zf:
            with zf.open("US.txt") as f:
                for line in tqdm(f, desc="Parsing GeoNames", unit=" rows"):
                    parts = line.decode("utf-8").rstrip("\n").split("\t")
                    if len(parts) < 19:
                        continue
                    row = dict(zip(COLUMNS, parts))
                    if row["feature_class"] not in WANTED_CLASSES:
                        continue
                    records.append(row)
        return records

    def normalize(self, parsed: list[dict]) -> list[dict]:
        out = []
        for r in parsed:
            try:
                lat = float(r["latitude"])
                lon = float(r["longitude"])
            except (ValueError, TypeError):
                continue
            alts = [a for a in (r.get("alternate_names") or "").split(",") if a]
            out.append({
                "geoname_id":       int(r["geoname_id"]),
                "name":             r["name"],
                "ascii_name":       r["ascii_name"] or None,
                "alternate_names":  alts or None,
                "latitude":         lat,
                "longitude":        lon,
                "feature_class":    r["feature_class"] or None,
                "feature_code":     r["feature_code"] or None,
                "country_code":     r["country_code"] or None,
                "admin1_code":      r["admin1_code"] or None,
                "admin2_code":      r["admin2_code"] or None,
                "admin3_code":      r["admin3_code"] or None,
                "admin4_code":      r["admin4_code"] or None,
                "population":       int(r["population"]) if r["population"] else 0,
                "elevation_m":      int(r["elevation_m"]) if r["elevation_m"] else None,
                "timezone":         r["timezone"] or None,
                "h3_r7":            self.h3_point(lat, lon, H3_R7),
                "h3_r9":            self.h3_point(lat, lon, H3_R9),
                "modification_date": r["modification_date"] or None,
            })
        return out

    def validate(self, records):
        valid = [r for r in records if r["geoname_id"] and r["latitude"] and r["longitude"]]
        invalid = [r for r in records if r not in valid]
        return valid, invalid

    def load(self, records: list[dict], conn) -> int:
        return upsert(conn, "world_geonames", records, ["geoname_id"])

    def run(self, conn):
        print(f"\n=== {self.source_name} ===")
        for url in self.discover():
            raw = self.fetch(url)
            parsed = self.parse(raw, url)
            normalized = self.normalize(parsed)
            valid, invalid = self.validate(normalized)
            print(f"  valid={len(valid)}  invalid={len(invalid)}")
            n = self.load(valid, conn)
            print(f"  loaded {n} rows → world_geonames")
